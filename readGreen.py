    ...
    for i in range(len(st) // 3):
        ...
        for j in range(len(gr)):
            ...
            if (difDist <= 0.0100):
                # station metadat
                # a
                gr[j].stats.network = st[i * 3].stats.network
                gr[j].stats.station = st[i * 3].stats.station
                gr[j].stats.dist = st[i * 3].stats.dist
                gr[j].stats.az = st[i * 3].stats.az
                gr[j].stats.baz = st[i * 3].stats.baz
                # gr[j].stats.gcarc = st[i].stats.gcarc
                gr[j].stats.stlo = st[i * 3].stats.stlo
                gr[j].stats.stla = st[i * 3].stats.stla
                gr[j].stats.evlo = st[i * 3].stats.evlo
                gr[j].stats.evla = st[i * 3].stats.evla
                # begin time
                gr[j].stats.starttime = st[i * 3].stats.starttime
                tmp = copy.deepcopy(gr[j])
                green.append(tmp)
                # fit
...
def aquireGreens(args, greenFile):
    ...
    h_1i = I.read_ints()
    h_1f = F.read_reals(numpy.float32)
    alpha = h_1f[0]
    depth = h_1f[1]
    fl = h_1f[2]
    fu = h_1f[3]
    ...
    # else do not.
    h_2f = F.read_reals(numpy.float32)  # Usealess for this line but neaded
    ...
    h_3f = F.read_reals(numpy.float32)
    h_3i = I.read_ints()
    beg = 0
    inc = 70
    d = h_3f[beg:beg + inc]
    ...
    # Vectors of float
    h_4f = F.read_reals(numpy.float32)
    h_4i = I.read_ints()
    beg = 0
    inc = Max_nr_dists
    ...
    for i in range(0, n2 + 0):
        h_5f = F.read_reals(numpy.float32)
        omega[i] = h_5f[0]

        for j in range(0, nrange + 0):

            for k in range(0, 10 + 0):
                FF = F.read_reals(numpy.float64)
    ...
    for j in range(0, nrange + 0):
        ...
        for k in range(0, 10 + 0):
            ...
            if (isrc[k] == 1):
                ...
                green.append(t)
    starttime = green[0].stats.starttime
    endtime = starttime + (int(args.npts)-1) * eval(args.delta) / 2

    green.trim(starttime=starttime, endtime=endtime)
